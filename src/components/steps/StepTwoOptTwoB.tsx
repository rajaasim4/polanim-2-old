...
import { StepTwoOptTwoBOptions } from "../../utils/options";
import { StepTwoOptTwoBAtom } from "../../store";

const StepTwoOptTwoB = () => {
  const [StepTwoOptTwoBoption, setStepTwoOptTwoBOptions] =
    useAtom(StepTwoOptTwoBAtom);
  ...
  return (
    <div className={cn(stepContainerStyles, "mb-4")}>
      ...
      <div className="text-xl flex gap-4 mb-4 flex-wrap justify-center max-w-full">
        {StepTwoOptTwoBOptions.map((option, index) => (
          <span
            key={index}
            className={cn(
              "py-4 px-2 rounded-lg bg-white cursor-pointer w-40 md:w-48 max-w-full text-center text-lg md:text-3xl select-none",
              {
                "bg-themeBlue text-white": StepTwoOptTwoBoption === option,
              }
            )}
            onClick={() => {
              setStepTwoOptTwoBOptions(option);
            }}
          >
            {option}
          </span>
